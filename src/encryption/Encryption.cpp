...
) const
{
    ...
    // 2. Derive 32-byte key from password using Argon2id via Monocypher v4 structs
    ...
    // 5. Convert combined bytes to hex string for file storage
    return toHex(outBytes);
}
...
) const
{
    ...
    std::vector<uint8_t> cipherBytes = fromHex(cipherText);
